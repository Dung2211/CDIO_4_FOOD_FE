                ...
                <template v-for="(value, index) in list_don_hang" :key="index">
                  <tr>
                    ...
                    <td class="text-center">
                      <span
                        v-if="
                          value.trang_thai_quan == -1 || value.tinh_trang == 4
                        "
      ...
      <div class="modal-content">
        <div class="modal-header text-white">
          <h5 class="modal-title">
            ...
            CHI TIẾT ĐƠN HÀNG -
            <b class="text-danger">{{ chi_tiet.ma_don_hang }}</b>
          </h5>
          <button
            type="button"
              ...
              <tbody>
                <template v-for="(value, index) in list_chi_tiet" :key="index">
                  <tr class="align-middle">
                    ...
                    <td class="text-end">
                      {{ value.ghi_chu ? value.ghi_chu : "-" }}
                    </td>
                    <td class="text-end fw-bold">
                      {{ formatVND(value.thanh_tien) }}
                    </td>
                  </tr>
                </template>
              </tbody>
...
export default {
  ...
  methods: {
        ...
        .then((res) => {
          ...
          this.list_don_hang.forEach((don) => {
            if (
              ...
              this.$toast.warning(
                `Đơn hàng ${don.ma_don_hang} đã bị quán hủy.`,
              );
    ...
    xemChiTiet(payload) {
      Object.assign(this.chi_tiet, payload);
      axios
        .post(
