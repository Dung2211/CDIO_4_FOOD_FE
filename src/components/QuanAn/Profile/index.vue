...
export default {
  ...
  methods: {
    ...
    // 🚀 THÊM MỚI: Xử lý upload ảnh qua ImgBB
    uploadAnh(event) {
      ...
      if (file) {
        ...
        // 🚀 ĐÃ SỬA: Dùng fetch() nguyên thủy để tránh bị Axios gắn nhầm Header
        fetch(`https://api.imgbb.com/1/upload?key=${apiKey}`, {
          ...
          .then((result) => {
            if (result.success) {
              // Cập nhật link ảnh mới vào biến user
              this.user.hinh_anh = result.data.url;
    ...
    doiMatKhau() {
      axios
        .post(
          "http://127.0.0.1:8000/api/quan-an/update-password",
          this.doi_mat_khau,
          {
            headers: {
